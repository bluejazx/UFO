...
//sets a psuedorandom codeword from a vector of words
std::string get_codeword()
{
  std::vector<std::string> codewords = {"strip", "refer", "proof", "motif", "laser", "funny", "evoke", "frank", "tribe", "swear"};
  srand(time(0));
  return codewords[rand() % (codewords.size() - 1)];
}
...
void game_over(std::string guess, std::string codeword)
{
  if ( guess == codeword)
  {
    std::cout << "Hooray! You saved your friend and the aliens have fled earth\n\n";
    std::cout << "The codeword wass " << codeword << "\n\n";
  }
  else
  {
...
//shows the numbers of incorrect letters along with ascII art
